import{g as M,s as T,D as z,a as F,M as K,b as O,W as D,e as j,c as R,d as V}from"./main-Bt4dk1WJ.js";function $(b,e,t){const n=e||"https://BACKEND-URLINIZ";return`<!-- Çark Çevir Kazan Widget -->
...
<div class="ikas-guide">
  ...
  <ul>
    ...
    `}setupEntriesListeners(){var e,t;this.loadEntries(),(e=document.getElementById("exportBtn"))==null||e.addEventListener("click",async()=>{const n=u();if(p()){const a=await(await fetch(`${n}/api/admin/entries/export`,{headers:{Authorization:`Bearer ${p()}`}})).blob(),s=URL.createObjectURL(a),o=document.createElement("a");o.href=s,o.download=`cark-katilimcilar-${new Date().toISOString().split("T")[0]}.csv`,document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(s)}else j();this.showToast("CSV dosyası indiriliyor")}),(t=document.getElementById("clearEntriesBtn"))==null||t.addEventListener("click",async()=>{if(!confirm("Tüm katılımcı verileri silinecek. Emin misiniz?"))return;const n=u();p()?await fetch(`${n}/api/admin/entries`,{method:"DELETE",headers:{Authorization:`Bearer ${p()}`}}):R(),this.entriesPage=1,this.loadEntries(),this.showToast("Veriler silindi")})}async loadEntries(){var l,c;const e=document.getElementById("entriesContainer"),t=u(),n=50;this.entriesPage=this.entriesPage||1;let i=[],a={total:0,today:0,mostWon:"-"},s=0;if(p())try{const r=p(),[m,d]=await Promise.all([fetch(`${t}/api/admin/entries?page=${this.entriesPage}&limit=${n}`,{headers:{Authorization:`Bearer ${r}`}}),fetch(`${t}/api/admin/stats`,{headers:{Authorization:`Bearer ${r}`}})]);if(m.ok){const y=await m.json();i=y.entries||[],s=y.total||0}d.ok&&(a=await d.json())}catch{}else{i=V();const r=new Date().toISOString().split("T")[0];a.total=i.length,a.today=i.filter(d=>{var y;return(y=d.timestamp)==null?void 0:y.startsWith(r)}).length;const m=i.map(d=>d.prize).filter(Boolean);if(m.length>0){const d=m.reduce((y,k)=>(y[k]=(y[k]||0)+1,y),{});a.mostWon=Object.keys(d).reduce((y,k)=>d[y]>d[k]?y:k)}}if(document.getElementById("stat-total").textContent=a.total,document.getElementById("stat-today").textContent=a.today,document.getElementById("stat-mostwon").textContent=a.mostWon,document.getElementById("stat-broken").textContent=a.brokenCoupons??"-",p()?s===0:i.length===0){e.innerHTML='<div class="empty-state">Henüz kimse çarkı çevirmedi.</div>';return}e.innerHTML=`
      ...
      `:""}
    `,(l=document.getElementById("entriesPrevBtn"))==null||l.addEventListener("click",()=>{this.entriesPage=Math.max(1,this.entriesPage-1),this.loadEntries()}),(c=document.getElementById("entriesNextBtn"))==null||c.addEventListener("click",()=>{this.entriesPage+=1,this.loadEntries()})}renderIntegrationTab(){var n;const e=$(this.config,u(),(n=this.store)==null?void 0:n.slug),t=N();return`
