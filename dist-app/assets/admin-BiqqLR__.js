import{g as T,s as w,D as x,a as $,e as z,c as L,b as A}from"./storage-F5Wv_NNO.js";function M(k,t,e){const a=t;return`<!-- Çark Çevir Kazan Widget -->
...
<div class="ikas-guide">
  ...
  <ul>
    ...
    `,document.getElementById("editModal").classList.add("active"),document.getElementById("seg-prob").addEventListener("input",a=>{document.getElementById("seg-prob-val").textContent=a.target.value}),document.getElementById("seg-type").addEventListener("change",a=>{const n=document.getElementById("seg-val-group"),i=document.getElementById("seg-coupon-group"),o=document.getElementById("seg-ikas-campaign-group"),s=a.target.value==="noLuck",l=a.target.value==="freeShipping";n&&(n.style.display=s||l?"none":"block"),i&&(i.style.display=s?"none":"block"),o&&(o.style.display=s?"none":"block")}),this.populateIkasCampaignSelect(e.ikasCampaignId),document.getElementById("cancelSegBtn").addEventListener("click",()=>document.getElementById("editModal").classList.remove("active")),document.getElementById("saveSegBtn").addEventListener("click",async()=>{var n,i,o;const a={id:this.editingSegmentId||$(),label:document.getElementById("seg-label").value||"Yeni Ödül",icon:document.getElementById("seg-icon").value||"",color:document.getElementById("seg-color").value||"#1E3A8A",textColor:document.getElementById("seg-textcolor").value||"#FFFFFF",discountType:document.getElementById("seg-type").value||"percentage",discountValue:parseInt((n=document.getElementById("seg-value"))==null?void 0:n.value)||0,couponCode:((i=document.getElementById("seg-coupon"))==null?void 0:i.value)||null,ikasCampaignId:((o=document.getElementById("seg-ikas-campaign"))==null?void 0:o.value)||null,probability:parseInt(document.getElementById("seg-prob").value)||10};if(this.editingSegmentId){const s=this.config.segments.findIndex(l=>String(l.id)===String(this.editingSegmentId));s!==-1&&(this.config.segments[s]=a)}else this.config.segments.push(a);document.getElementById("editModal").classList.remove("active"),await this.saveAndRender({segments:this.config.segments})})}async fetchIkasCampaigns(){if(this._ikasCampaigns)return this._ikasCampaigns;const t=m();try{const e=await fetch(`${t}/api/admin/ikas/campaigns`,{headers:{Authorization:`Bearer ${c()}`}});if(e.ok){const a=await e.json();return this._ikasCampaigns=a.campaigns||[],this._ikasCampaigns}}catch{}return[]}async populateIkasCampaignSelect(t){const e=document.getElementById("seg-ikas-campaign"),a=document.getElementById("seg-ikas-campaign-hint");if(!e)return;const n=await this.fetchIkasCampaigns(),i=document.getElementById("seg-ikas-campaign");if(i){if(n.length===0){if(a){a.innerHTML=`İkas kampanyası bulunamadı. Backend az önce uyandıysa (Render ücretsiz plan) birkaç saniye sürebilir — <a href="#" id="retryIkasCampaigns" style="color:var(--cark-primary,#ffd700);text-decoration:underline;">tekrar dene</a>. Yoksa İkas Builder'dan bir kampanya oluşturup buradan seçebilir, ya da yukarıya sabit bir kupon kodu girebilirsiniz.`;const o=document.getElementById("retryIkasCampaigns");o&&o.addEventListener("click",s=>{s.preventDefault(),a.textContent="Yükleniyor...",this.populateIkasCampaignSelect(t)})}return}n.forEach(o=>{const s=document.createElement("option");s.value=o.id,s.textContent=o.title+(o.hasCoupon?"":" (kuponsuz kampanya — önce İkas'ta kupon özelliğini açın)"),String(o.id)===String(t)&&(s.selected=!0),i.appendChild(s)})}}drawPreviewWheel(t="previewCanvas",e=null){const a=document.getElementById(t);if(!a)return;const n={...x.theme,...this.config.theme||{},...e||{}},i=a.getContext("2d"),o=a.width/2,s=a.height/2,l=Math.min(o,s)-10;if(i.clearRect(0,0,a.width,a.height),!this.config.segments.length)return;const d=this.config.segments.reduce((g,r)=>g+r.probability,0)||1;let u=-Math.PI/2;const y=document.getElementById("previewStats");y&&(y.innerHTML=`Toplam Ağırlık: <span>${d}</span>`),i.beginPath(),i.arc(o,s,l,0,Math.PI*2),i.fillStyle="#1a1a2e",i.fill(),i.strokeStyle=K(n.primaryColor,.3),i.lineWidth=2,i.stroke();for(const g of this.config.segments){const r=g.probability/d*2*Math.PI,v=u+r;i.beginPath(),i.moveTo(o,s),i.arc(o,s,l-10,u,v),i.closePath(),i.fillStyle=g.color,i.fill(),i.strokeStyle="rgba(255,255,255,0.3)",i.lineWidth=1,i.stroke();const h=u+r/2;i.save(),i.translate(o,s),i.rotate(h),i.textAlign="center",i.textBaseline="middle",i.font="bold 12px sans-serif",i.fillStyle=g.textColor||"#FFF",i.fillText(g.label||"",l*.6,0),i.restore(),u=v}i.beginPath(),i.arc(o,s,l*.2,0,Math.PI*2),i.fillStyle=n.bgDark,i.fill(),i.strokeStyle=n.primaryColor,i.lineWidth=2,i.stroke()}renderEntriesTab(){return`
    ...
    `}setupEntriesListeners(){var t,e;this.loadEntries(),(t=document.getElementById("exportBtn"))==null||t.addEventListener("click",()=>{const a=m();c()?window.open(`${a}/api/admin/entries/export?token=${c()}`,"_blank"):z(),this.showToast("CSV dosyası indiriliyor")}),(e=document.getElementById("clearEntriesBtn"))==null||e.addEventListener("click",async()=>{if(!confirm("Tüm katılımcı verileri silinecek. Emin misiniz?"))return;const a=m();c()?await fetch(`${a}/api/admin/entries`,{method:"DELETE",headers:{Authorization:`Bearer ${c()}`}}):L(),this.loadEntries(),this.showToast("Veriler silindi")})}async loadEntries(){const t=document.getElementById("entriesContainer"),e=m();let a=[],n={total:0,today:0,mostWon:"-"};if(c())try{const i=c(),[o,s]=await Promise.all([fetch(`${e}/api/admin/entries?limit=500`,{headers:{Authorization:`Bearer ${i}`}}),fetch(`${e}/api/admin/stats`,{headers:{Authorization:`Bearer ${i}`}})]);o.ok&&(a=(await o.json()).entries||[]),s.ok&&(n=await s.json())}catch{}else{a=A();const i=new Date().toISOString().split("T")[0];n.total=a.length,n.today=a.filter(s=>{var l;return(l=s.timestamp)==null?void 0:l.startsWith(i)}).length;const o=a.map(s=>s.prize).filter(Boolean);if(o.length>0){const s=o.reduce((l,d)=>(l[d]=(l[d]||0)+1,l),{});n.mostWon=Object.keys(s).reduce((l,d)=>s[l]>s[d]?l:d)}}if(document.getElementById("stat-total").textContent=n.total,document.getElementById("stat-today").textContent=n.today,document.getElementById("stat-mostwon").textContent=n.mostWon,a.length===0){t.innerHTML='<div class="empty-state">Henüz kimse çarkı çevirmedi.</div>';return}t.innerHTML=`
    ...
    `}renderIntegrationTab(){var a;const t=M(this.config,m(),(a=this.store)==null?void 0:a.slug),e=F();return`
