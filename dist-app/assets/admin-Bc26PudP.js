import{g as F,s as z,D as $,a as L,M as K,b as O,W as D,e as G,c as j,d as R}from"./main-Bt4dk1WJ.js";function A(b,e,n){const t=e||"https://BACKEND-URLINIZ";return`<!-- Çark Çevir Kazan Widget -->
...
<div class="ikas-guide">
  ...
  <ul>
                ...
                ${e.map((n,t)=>`
                  <div class="segment-item" data-id="${n.couponGroupId}">
                    ...
                    <div class="segment-info">
                      <div class="segment-label" style="color:${n.textColor||"#fff"}">${h(n.icon)} ${h(n.label)}</div>
                      <div class="segment-meta">Çarkta ${n.sliceCount} dilim • Kazanma ağırlığı: %${Number(n.probability.toFixed(1))} ${n.couponCode?`• Kod: ${h(n.couponCode)}`:""} ${n.ikasCampaignId?"• İkas kampanyasına bağlı":""}</div>
                    </div>
                    <div class="segment-actions">
                      ...
                      ${n.discountType!=="noLuck"?`<button class="test-coupon-btn" data-id="${h(n.id)}" title="Bu kupon gerçek bir müşteri kazanmadan İkas'ta üretilebiliyor mu test et">🧪</button>`:""}
    ...
    `,this.openModal("editModal"),document.getElementById("seg-prob").addEventListener("input",a=>{document.getElementById("seg-prob-val").textContent=a.target.value}),document.getElementById("seg-type").addEventListener("change",a=>{const i=document.getElementById("seg-val-group"),o=document.getElementById("seg-coupon-group"),l=document.getElementById("seg-ikas-campaign-group"),s=a.target.value==="noLuck",d=a.target.value==="freeShipping";i&&(i.style.display=s||d?"none":"block"),o&&(o.style.display=s?"none":"block"),l&&(l.style.display=s?"none":"block")}),this.populateIkasCampaignSelect(t.ikasCampaignId),document.getElementById("cancelSegBtn").addEventListener("click",()=>this.closeModal("editModal")),document.getElementById("saveSegBtn").addEventListener("click",async()=>{var o,l,s;const a={id:t.id||L(),couponGroupId:t.couponGroupId,label:document.getElementById("seg-label").value||"Yeni Ödül",icon:document.getElementById("seg-icon").value||"",color:document.getElementById("seg-color").value||"#1E3A8A",textColor:document.getElementById("seg-textcolor").value||"#FFFFFF",discountType:document.getElementById("seg-type").value||"percentage",discountValue:parseInt((o=document.getElementById("seg-value"))==null?void 0:o.value)||0,couponCode:((l=document.getElementById("seg-coupon"))==null?void 0:l.value)||null,ikasCampaignId:((s=document.getElementById("seg-ikas-campaign"))==null?void 0:s.value)||null,probability:parseInt(document.getElementById("seg-prob").value)||10},i=this.getCouponTemplates();if(this.editingSegmentId){const d=i.findIndex(r=>String(r.couponGroupId)===String(this.editingSegmentId));d!==-1&&(i[d]={...a,sliceCount:i[d].sliceCount})}else{if(i.length>=6){this.showToast("En fazla 6 farklı kupon tanıtabilirsiniz","error");return}i.push({...a,sliceCount:0})}this.config.segments=this.distributeCouponsToSixSlices(i),this.closeModal("editModal"),await this.saveAndRender({segments:this.config.segments})})}async fetchIkasCampaigns(){if(this._ikasCampaigns)return this._ikasCampaigns;const e=g();try{const n=await fetch(`${e}/api/admin/ikas/campaigns`,{headers:{Authorization:`Bearer ${p()}`}});if(n.ok){const t=await n.json();return this._ikasCampaigns=t.campaigns||[],this._ikasCampaigns}}catch{}return[]}async populateIkasCampaignSelect(e,n=!1){const t=document.getElementById("seg-ikas-campaign"),a=document.getElementById("seg-ikas-campaign-hint");if(!t)return;const i=await this.fetchIkasCampaigns(),o=document.getElementById("seg-ikas-campaign");if(o){if(i.length===0){if(!n){a&&(a.textContent="Yükleniyor... (backend uyanıyor olabilir)"),this._ikasCampaigns=null,setTimeout(()=>this.populateIkasCampaignSelect(e,!0),4e3);return}if(a){a.innerHTML=`Kuponu olan bir İkas kampanyası bulunamadı (kuponsuz kampanyalar burada listelenmez). <a href="#" id="retryIkasCampaigns" style="color:var(--cark-primary,#ffd700);text-decoration:underline;">tekrar dene</a>. Yoksa İkas Builder'da kampanyanıza bir kupon kodu ekleyip buradan seçebilir, ya da (önerilen) yukarıya sabit bir kupon kodu girebilirsiniz.`;const l=document.getElementById("retryIkasCampaigns");l&&l.addEventListener("click",s=>{s.preventDefault(),a.textContent="Yükleniyor...",this._ikasCampaigns=null,this.populateIkasCampaignSelect(e,!0)})}return}i.forEach(l=>{const s=document.createElement("option");s.value=l.id,s.textContent=l.title,String(l.id)===String(e)&&(s.selected=!0),o.appendChild(s)})}}renderLivePreview(e,n=null){const t=document.getElementById(e);if(!t)return;t.innerHTML="";const a=this.config.segments.reduce((d,r)=>d+r.probability,0)||1,i=document.getElementById("previewStats");if(i&&(i.innerHTML=`Toplam Ağırlık: <span>${a}</span>`),!this.config.segments.length)return;const o={...this.config,theme:{...$.theme,...this.config.theme||{},...n||{}}},s=new K(o).buildDOM(t);O(document.getElementById("cark-widget-root"),o.theme),new D(s.canvas,o)}renderEntriesTab(){return`
    ...
    `}setupEntriesListeners(){var e,n;this.loadEntries(),(e=document.getElementById("exportBtn"))==null||e.addEventListener("click",async()=>{const t=g();if(p()){const i=await(await fetch(`${t}/api/admin/entries/export`,{headers:{Authorization:`Bearer ${p()}`}})).blob(),o=URL.createObjectURL(i),l=document.createElement("a");l.href=o,l.download=`cark-katilimcilar-${new Date().toISOString().split("T")[0]}.csv`,document.body.appendChild(l),l.click(),document.body.removeChild(l),URL.revokeObjectURL(o)}else G();this.showToast("CSV dosyası indiriliyor")}),(n=document.getElementById("clearEntriesBtn"))==null||n.addEventListener("click",async()=>{if(!confirm("Tüm katılımcı verileri silinecek. Emin misiniz?"))return;const t=g();p()?await fetch(`${t}/api/admin/entries`,{method:"DELETE",headers:{Authorization:`Bearer ${p()}`}}):j(),this.entriesPage=1,this.loadEntries(),this.showToast("Veriler silindi")})}async loadEntries(){var s,d;const e=document.getElementById("entriesContainer"),n=g(),t=50;this.entriesPage=this.entriesPage||1;let a=[],i={total:0,today:0,mostWon:"-"},o=0;if(p())try{const r=p(),[m,c]=await Promise.all([fetch(`${n}/api/admin/entries?page=${this.entriesPage}&limit=${t}`,{headers:{Authorization:`Bearer ${r}`}}),fetch(`${n}/api/admin/stats`,{headers:{Authorization:`Bearer ${r}`}})]);if(m.ok){const u=await m.json();a=u.entries||[],o=u.total||0}c.ok&&(i=await c.json())}catch{}else{a=R();const r=new Date().toISOString().split("T")[0];i.total=a.length,i.today=a.filter(c=>{var u;return(u=c.timestamp)==null?void 0:u.startsWith(r)}).length;const m=a.map(c=>c.prize).filter(Boolean);if(m.length>0){const c=m.reduce((u,k)=>(u[k]=(u[k]||0)+1,u),{});i.mostWon=Object.keys(c).reduce((u,k)=>c[u]>c[k]?u:k)}}if(document.getElementById("stat-total").textContent=i.total,document.getElementById("stat-today").textContent=i.today,document.getElementById("stat-mostwon").textContent=i.mostWon,document.getElementById("stat-broken").textContent=i.brokenCoupons??"-",p()?o===0:a.length===0){e.innerHTML='<div class="empty-state">Henüz kimse çarkı çevirmedi.</div>';return}e.innerHTML=`
      ...
      `:""}
    `,(s=document.getElementById("entriesPrevBtn"))==null||s.addEventListener("click",()=>{this.entriesPage=Math.max(1,this.entriesPage-1),this.loadEntries()}),(d=document.getElementById("entriesNextBtn"))==null||d.addEventListener("click",()=>{this.entriesPage+=1,this.loadEntries()})}renderIntegrationTab(){var t;const e=A(this.config,g(),(t=this.store)==null?void 0:t.slug),n=N();return`
