...
<div class="ikas-guide">
  ...
  <ul>
                ...
                ${e.map((t,a)=>`
                  <div class="segment-item" data-id="${t.couponGroupId}">
                    ...
                    <div class="segment-info">
                      ...
                      <div class="segment-meta">Çarkta ${t.sliceCount} dilim • Kazanma ağırlığı: %${Number(t.probability.toFixed(1))} ${t.couponCode?`• Kod: ${p(t.couponCode)}`:""} ${t.ikasCampaignId?`• ${p(V(t)||"İkas kampanyasına bağlı")}`:""}</div>
                    </div>
    ...
    `}setupEntriesListeners(){var a,n,i,s,r,o,d,c,l,u,k,b,B,g;this.entriesPage=this.entriesPage||1,this.entriesPageSize=this.entriesPageSize||25,this.entriesFilters=this.entriesFilters||{dateFrom:"",dateTo:"",prize:"",status:"",search:""},this.selectedEntryIds=new Set,this.loadEntries();const e=()=>{this.entriesFilters={dateFrom:document.getElementById("entriesDateFrom").value,dateTo:document.getElementById("entriesDateTo").value,prize:document.getElementById("entriesPrizeFilter").value,status:document.getElementById("entriesStatusFilter").value,search:document.getElementById("entriesSearch").value.trim()},this.entriesPage=1,this.selectedEntryIds.clear(),this.loadEntries()};["entriesDateFrom","entriesDateTo","entriesPrizeFilter","entriesStatusFilter"].forEach(v=>{var m;(m=document.getElementById(v))==null||m.addEventListener("change",e)});let t;(a=document.getElementById("entriesSearch"))==null||a.addEventListener("input",()=>{clearTimeout(t),t=setTimeout(e,350)}),(n=document.getElementById("resetEntriesFiltersBtn"))==null||n.addEventListener("click",()=>{this.entriesFilters={dateFrom:"",dateTo:"",prize:"",status:"",search:""},["entriesDateFrom","entriesDateTo","entriesPrizeFilter","entriesStatusFilter","entriesSearch"].forEach(v=>{const m=document.getElementById(v);m&&(m.value="")}),this.entriesPage=1,this.selectedEntryIds.clear(),this.loadEntries()}),(i=document.getElementById("exportBtn"))==null||i.addEventListener("click",()=>this.downloadEntries("csv")),(s=document.getElementById("exportExcelBtn"))==null||s.addEventListener("click",()=>this.downloadEntries("excel")),(r=document.getElementById("exportBrokenBtn"))==null||r.addEventListener("click",()=>this.downloadEntries("csv",{status:"failed"})),(o=document.getElementById("showBrokenBtn"))==null||o.addEventListener("click",()=>{var v;document.getElementById("entriesStatusFilter").value="failed",e(),(v=document.getElementById("entriesContainer"))==null||v.scrollIntoView({behavior:"smooth",block:"start"})}),(d=document.getElementById("retryBrokenBtn"))==null||d.addEventListener("click",()=>this.retryAllBrokenEntries()),(c=document.getElementById("clearEntriesBtn"))==null||c.addEventListener("click",async()=>{if(!confirm("Tüm katılımcı verileri silinecek. Emin misiniz?"))return;const v=y(),m=await fetch(`${v}/api/admin/entries`,{method:"DELETE",headers:{Authorization:`Bearer ${h()}`}}),f=await m.json().catch(()=>({}));if(!m.ok){this.showToast(f.error||"Katılımcılar silinemedi","error");return}this.entriesPage=1,this.loadEntries(),this.showToast("Veriler silindi")}),(l=document.getElementById("entriesBulkToolbar"))==null||l.addEventListener("click",v=>{var f;const m=(f=v.target.closest("[data-bulk-action]"))==null?void 0:f.dataset.bulkAction;m&&this.handleEntriesBulkAction(m)}),(u=document.getElementById("createTestEntryBtn"))==null||u.addEventListener("click",()=>this.createTestEntry()),(k=document.getElementById("checkWidgetStatusBtn"))==null||k.addEventListener("click",()=>this.checkWidgetStatus()),(b=document.getElementById("openInstallGuideBtn"))==null||b.addEventListener("click",()=>{var v;(v=document.querySelector('.admin-nav a[data-tab="integration"]'))==null||v.click()}),(B=document.getElementById("closeEntryDetailBtn"))==null||B.addEventListener("click",()=>this.closeEntryDetail()),(g=document.getElementById("entryDetailScrim"))==null||g.addEventListener("click",()=>this.closeEntryDetail())}async loadEntries(){var u,k,b,B,g,v;const e=document.getElementById("entriesContainer");if(!e)return;const t=y(),a=this.entriesPageSize||25;e.innerHTML='<div class="entries-loading-state"><div class="entries-spinner"></div><span>Katılımcılar yükleniyor...</span></div>';let n,i,s,r;if(h())try{const m=h(),f=this.entriesQueryParams();f.set("page",this.entriesPage||1),f.set("limit",a);const[w,I]=await Promise.all([fetch(`${t}/api/admin/entries?${f}`,{headers:{Authorization:`Bearer ${m}`}}),fetch(`${t}/api/admin/stats`,{headers:{Authorization:`Bearer ${m}`}})]);if(!w.ok||!I.ok)throw new Error("Katılımcılar yüklenemedi");const S=await w.json();n=S.entries||[],s=S.total||0,r=S.prizes||[],i=await I.json()}catch(m){e.innerHTML=`<div class="entries-error-state"><strong>Katılımcılar yüklenemedi</strong><span>${p(m.message)}</span><button class="btn btn-secondary" id="retryEntriesLoadBtn">Tekrar dene</button></div>`,(u=document.getElementById("retryEntriesLoadBtn"))==null||u.addEventListener("click",()=>this.loadEntries());return}else{e.innerHTML='<div class="entries-error-state"><strong>Oturum bulunamadı</strong><span>Katılımcı verilerini görmek için tekrar giriş yapın.</span></div>';return}this.currentEntries=n,this.currentEntryMap=new Map(n.map(m=>[String(m.id),m]));const o=document.getElementById("entriesPrizeFilter");if(o){const m=((k=this.entriesFilters)==null?void 0:k.prize)||"";o.innerHTML=`<option value="">Tüm ödüller</option>${r.map(f=>`<option value="${p(f)}">${p(f)}</option>`).join("")}`,o.value=m}document.getElementById("stat-total").textContent=i.total,document.getElementById("stat-today").textContent=i.today,document.getElementById("stat-mostwon").textContent=i.mostWon,document.getElementById("stat-processed").textContent=i.processed??0,document.getElementById("stat-broken").textContent=i.failed??i.brokenCoupons??0,document.getElementById("stat-conversion").textContent=`%${i.conversionRate??0}`;const d=i.failed??i.brokenCoupons??0,c=document.getElementById("entriesIssueBanner");if(c.hidden=d===0,document.getElementById("entriesIssueCount").textContent=d,this.renderPrizeDistribution(i.prizeDistribution||[]),h()?s===0:n.length===0){const m=Object.values(this.entriesFilters||{}).some(Boolean);e.innerHTML=m?'<div class="entries-empty-state"><div>🔎</div><strong>Filtrelere uygun katılımcı bulunamadı</strong><span>Filtreleri değiştirerek tekrar deneyin.</span><button class="btn btn-secondary" id="emptyResetFiltersBtn">Filtreleri temizle</button></div>':'<div class="entries-empty-state"><div>🎡</div><strong>Henüz katılımcı yok</strong><span>Çark sitenize eklendikten sonra katılımlar burada görünür.</span></div>',(b=document.getElementById("emptyResetFiltersBtn"))==null||b.addEventListener("click",()=>{var f;return(f=document.getElementById("resetEntriesFiltersBtn"))==null?void 0:f.click()});return}e.innerHTML=`
      <table class="entries-table">
        ...
        <tbody>
          ${n.map(m=>`
            <tr class="entry-row" data-entry-id="${p(m.id)}" tabindex="0">
              <td><input type="checkbox" class="entry-select" value="${p(m.id)}" ${this.selectedEntryIds.has(String(m.id))?"checked":""} aria-label="${p(m.name||"Katılımcı")} kaydını seç"></td>
              <td>${m.timestamp?new Date(m.timestamp).toLocaleString("tr-TR"):"-"}</td>
              <td>${p(m.name)||"-"}</td>
              <td><span class="masked-value" data-field="phone">${p(this.maskPhone(m.phone))||"-"}</span>${m.phone?'<button class="reveal-entry-value" data-field="phone" title="Telefonu göster">Göster</button>':""}</td>
              <td><span class="masked-value" data-field="email">${p(this.maskEmail(m.email))||"-"}</span>${m.email?'<button class="reveal-entry-value" data-field="email" title="E-postayı göster">Göster</button>':""}</td>
              <td class="entry-prize-cell">${p(m.prize)||"-"}</td>
              <td>${m.couponCode?`<code>${p(m.couponCode)}</code>`:"-"}</td>
              <td>${this.renderEntryStatus(m)}</td>
