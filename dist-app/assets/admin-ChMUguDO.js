import"./modulepreload-polyfill-B5Qt9EMX.js";import{D as C,g as x,M as D,a as O,W as K}from"./main-DRFRtDkY.js";const N="carkAdminConfig:";function z(f){return JSON.parse(JSON.stringify(f))}function A(f){if(!f)throw new Error("Mağaza kimliği olmadan admin önbelleği kullanılamaz");return`${N}${f}`}function G(f,e,t){try{const n=f.getItem(A(e));return n?JSON.parse(n):z(t)}catch{return z(t)}}function H(f,e,t){f.setItem(A(e),JSON.stringify(t))}function j(f,e={}){return f?{discountType:f.isFreeShipping?"freeShipping":"ikasCampaign",discountValue:null}:{discountType:e.discountType||"percentage",discountValue:Number.isFinite(e.discountValue)?e.discountValue:0}}function R({discountType:f,discountValue:e}={}){return f==="freeShipping"?"Ücretsiz Kargo":f==="percentage"&&Number.isFinite(Number(e))?`%${Number(e)}`:f==="fixed"&&Number.isFinite(Number(e))?`${Number(e)} TL`:f==="ikasCampaign"?"İndirim değeri İkas kampanyası tarafından belirlenir":""}function P(f,e,t){const n=e||"https://BACKEND-URLINIZ";return`<!-- Çark Çevir Kazan Widget -->
...
<div class="ikas-guide">
  ...
  <ul>
                ...
                ${e.map((t,n)=>`
                  <div class="segment-item" data-id="${t.couponGroupId}">
                    ...
                    <div class="segment-info">
                      ...
                      <div class="segment-meta">Çarkta ${t.sliceCount} dilim • Kazanma ağırlığı: %${Number(t.probability.toFixed(1))} ${t.couponCode?`• Kod: ${p(t.couponCode)}`:""} ${t.ikasCampaignId?`• ${p(R(t)||"İkas kampanyasına bağlı")}`:""}</div>
                    </div>
    ...
    `}setupEntriesListeners(){var n,a,i,s,o,r,c,l,m,u,k,E,B,y;this.entriesPage=this.entriesPage||1,this.entriesPageSize=this.entriesPageSize||25,this.entriesFilters=this.entriesFilters||{dateFrom:"",dateTo:"",prize:"",status:"",search:""},this.selectedEntryIds=new Set,this.loadEntries();const e=()=>{this.entriesFilters={dateFrom:document.getElementById("entriesDateFrom").value,dateTo:document.getElementById("entriesDateTo").value,prize:document.getElementById("entriesPrizeFilter").value,status:document.getElementById("entriesStatusFilter").value,search:document.getElementById("entriesSearch").value.trim()},this.entriesPage=1,this.selectedEntryIds.clear(),this.loadEntries()};["entriesDateFrom","entriesDateTo","entriesPrizeFilter","entriesStatusFilter"].forEach(b=>{var d;(d=document.getElementById(b))==null||d.addEventListener("change",e)});let t;(n=document.getElementById("entriesSearch"))==null||n.addEventListener("input",()=>{clearTimeout(t),t=setTimeout(e,350)}),(a=document.getElementById("resetEntriesFiltersBtn"))==null||a.addEventListener("click",()=>{this.entriesFilters={dateFrom:"",dateTo:"",prize:"",status:"",search:""},["entriesDateFrom","entriesDateTo","entriesPrizeFilter","entriesStatusFilter","entriesSearch"].forEach(b=>{const d=document.getElementById(b);d&&(d.value="")}),this.entriesPage=1,this.selectedEntryIds.clear(),this.loadEntries()}),(i=document.getElementById("exportBtn"))==null||i.addEventListener("click",()=>this.downloadEntries("csv")),(s=document.getElementById("exportExcelBtn"))==null||s.addEventListener("click",()=>this.downloadEntries("excel")),(o=document.getElementById("exportBrokenBtn"))==null||o.addEventListener("click",()=>this.downloadEntries("csv",{status:"failed"})),(r=document.getElementById("showBrokenBtn"))==null||r.addEventListener("click",()=>{var b;document.getElementById("entriesStatusFilter").value="failed",e(),(b=document.getElementById("entriesContainer"))==null||b.scrollIntoView({behavior:"smooth",block:"start"})}),(c=document.getElementById("retryBrokenBtn"))==null||c.addEventListener("click",()=>this.retryAllBrokenEntries()),(l=document.getElementById("clearEntriesBtn"))==null||l.addEventListener("click",async()=>{if(!confirm("Tüm katılımcı verileri silinecek. Emin misiniz?"))return;const b=h(),d=await fetch(`${b}/api/admin/entries`,{method:"DELETE",headers:{Authorization:`Bearer ${v()}`}}),g=await d.json().catch(()=>({}));if(!d.ok){this.showToast(g.error||"Katılımcılar silinemedi","error");return}this.entriesPage=1,this.loadEntries(),this.showToast("Veriler silindi")}),(m=document.getElementById("entriesBulkToolbar"))==null||m.addEventListener("click",b=>{var g;const d=(g=b.target.closest("[data-bulk-action]"))==null?void 0:g.dataset.bulkAction;d&&this.handleEntriesBulkAction(d)}),(u=document.getElementById("createTestEntryBtn"))==null||u.addEventListener("click",()=>this.createTestEntry()),(k=document.getElementById("checkWidgetStatusBtn"))==null||k.addEventListener("click",()=>this.checkWidgetStatus()),(E=document.getElementById("openInstallGuideBtn"))==null||E.addEventListener("click",()=>{var b;(b=document.querySelector('.admin-nav a[data-tab="integration"]'))==null||b.click()}),(B=document.getElementById("closeEntryDetailBtn"))==null||B.addEventListener("click",()=>this.closeEntryDetail()),(y=document.getElementById("entryDetailScrim"))==null||y.addEventListener("click",()=>this.closeEntryDetail())}async loadEntries(){var u,k,E,B,y,b;const e=document.getElementById("entriesContainer");if(!e)return;const t=h(),n=this.entriesPageSize||25;e.innerHTML='<div class="entries-loading-state"><div class="entries-spinner"></div><span>Katılımcılar yükleniyor...</span></div>';let a,i,s,o;if(v())try{const d=v(),g=this.entriesQueryParams();g.set("page",this.entriesPage||1),g.set("limit",n);const[w,I]=await Promise.all([fetch(`${t}/api/admin/entries?${g}`,{headers:{Authorization:`Bearer ${d}`}}),fetch(`${t}/api/admin/stats`,{headers:{Authorization:`Bearer ${d}`}})]);if(!w.ok||!I.ok)throw new Error("Katılımcılar yüklenemedi");const S=await w.json();a=S.entries||[],s=S.total||0,o=S.prizes||[],i=await I.json()}catch(d){e.innerHTML=`<div class="entries-error-state"><strong>Katılımcılar yüklenemedi</strong><span>${p(d.message)}</span><button class="btn btn-secondary" id="retryEntriesLoadBtn">Tekrar dene</button></div>`,(u=document.getElementById("retryEntriesLoadBtn"))==null||u.addEventListener("click",()=>this.loadEntries());return}else{e.innerHTML='<div class="entries-error-state"><strong>Oturum bulunamadı</strong><span>Katılımcı verilerini görmek için tekrar giriş yapın.</span></div>';return}this.currentEntries=a,this.currentEntryMap=new Map(a.map(d=>[String(d.id),d]));const r=document.getElementById("entriesPrizeFilter");if(r){const d=((k=this.entriesFilters)==null?void 0:k.prize)||"";r.innerHTML=`<option value="">Tüm ödüller</option>${o.map(g=>`<option value="${p(g)}">${p(g)}</option>`).join("")}`,r.value=d}document.getElementById("stat-total").textContent=i.total,document.getElementById("stat-today").textContent=i.today,document.getElementById("stat-mostwon").textContent=i.mostWon,document.getElementById("stat-processed").textContent=i.processed??0,document.getElementById("stat-broken").textContent=i.failed??i.brokenCoupons??0,document.getElementById("stat-conversion").textContent=`%${i.conversionRate??0}`;const c=i.failed??i.brokenCoupons??0,l=document.getElementById("entriesIssueBanner");if(l.hidden=c===0,document.getElementById("entriesIssueCount").textContent=c,this.renderPrizeDistribution(i.prizeDistribution||[]),v()?s===0:a.length===0){const d=Object.values(this.entriesFilters||{}).some(Boolean);e.innerHTML=d?'<div class="entries-empty-state"><div>🔎</div><strong>Filtrelere uygun katılımcı bulunamadı</strong><span>Filtreleri değiştirerek tekrar deneyin.</span><button class="btn btn-secondary" id="emptyResetFiltersBtn">Filtreleri temizle</button></div>':'<div class="entries-empty-state"><div>🎡</div><strong>Henüz katılımcı yok</strong><span>Çark sitenize eklendikten sonra katılımlar burada görünür.</span></div>',(E=document.getElementById("emptyResetFiltersBtn"))==null||E.addEventListener("click",()=>{var g;return(g=document.getElementById("resetEntriesFiltersBtn"))==null?void 0:g.click()});return}e.innerHTML=`
