import{g as T,s as w,D as C,a as $,e as L,c as A,b as P}from"./storage-D_1Co3aL.js";function M(k,t,e){const i=t;return`<!-- Çark Çevir Kazan Widget -->
...
<div class="ikas-guide">
  ...
  <ul>
...
</div>`}function g(){return window.CARK_API_URL||"https://cark-backend.onrender.com"}function u(){return localStorage.getItem("cark_admin_token")||""}const K={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"};function y(k){return String(k??"").replace(/[&<>"']/g,t=>K[t])}function D(k,t){const e=parseInt(k.replace("#",""),16),i=e>>16&255,n=e>>8&255,a=e&255;return`rgba(${i},${n},${a},${t})`}class R{constructor(){this.config=T(),this.store=null,this.currentTab="settings",this.editingSegmentId=null,this.authMode="login",this.init()}async init(){const t=u();if(t){const e=g();try{const i=await fetch(`${e}/api/auth/me`,{headers:{Authorization:`Bearer ${t}`}});if(i.ok){const n=await i.json();this.store=n.store,this.showContent(),this.loadFromBackend();return}}catch{}localStorage.removeItem("cark_admin_token")}this.showAuthForm("login")}showContent(){var n;const t=document.getElementById("adminPasswordOverlay"),e=document.getElementById("adminContent");t&&(t.style.display="none"),e&&(e.style.display="block");const i=document.getElementById("adminStoreName");i&&this.store&&(i.textContent=`— ${this.store.name}`),(n=document.getElementById("logoutBtn"))==null||n.addEventListener("click",()=>this.logout()),this.setupTabs(),this.render()}showAuthForm(t){this.authMode=t;const e=document.getElementById("adminPasswordOverlay");if(!e)return;e.style.display="flex";const i=document.getElementById("authTitle"),n=document.getElementById("authSubtitle"),a=document.getElementById("authFieldStoreName"),o=document.getElementById("authStoreName"),s=document.getElementById("authEmail"),r=document.getElementById("authPassword"),m=document.getElementById("adminPasswordError"),d=document.getElementById("authSubmitBtn"),v=document.getElementById("authSwitchToRegisterWrap"),c=document.getElementById("authSwitchToLoginWrap"),l=t==="register";i.textContent=l?"Mağaza Oluştur":"Giriş Yap",n.textContent=l?"Kendi çark widget hesabınızı oluşturun":"Mağazanızın admin paneline giriş yapın",a.style.display=l?"block":"none",d.textContent=l?"Hesap Oluştur":"Giriş Yap",v.style.display=l?"none":"inline",c.style.display=l?"inline":"none",m.style.display="none",document.getElementById("authSwitchToRegister").onclick=h=>{h.preventDefault(),this.showAuthForm("register")},document.getElementById("authSwitchToLogin").onclick=h=>{h.preventDefault(),this.showAuthForm("login")};const p=h=>{m.style.display="block",m.textContent=h},b=async()=>{const h=g(),f=s.value.trim(),E=r.value,I=o.value.trim();if(!f||!E||l&&!I){p("Lütfen tüm alanları doldurun");return}d.disabled=!0;try{const S=l?"/api/auth/register":"/api/auth/login",z=l?{storeName:I,email:f,password:E}:{email:f,password:E},x=await fetch(`${h}${S}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(z)}),B=await x.json().catch(()=>({}));if(!x.ok){p(B.error||"Bir hata oluştu");return}localStorage.setItem("cark_admin_token",B.token),this.store=B.store,this.showContent(),this.loadFromBackend()}catch{p("Backend bağlantı hatası")}finally{d.disabled=!1}};d.onclick=b,r.onkeydown=h=>{h.key==="Enter"&&b()},(l?o:s).focus()}logout(){localStorage.removeItem("cark_admin_token"),this.store=null,document.getElementById("adminContent").style.display="none",this.showAuthForm("login")}async loadFromBackend(){const t=g();try{const e=await fetch(`${t}/api/admin/config`,{headers:{Authorization:`Bearer ${u()}`}});e.ok&&(this.config=await e.json(),w(this.config),this.render())}catch{}}setupTabs(){document.querySelectorAll(".admin-nav a").forEach(t=>{t.addEventListener("click",e=>{e.preventDefault(),document.querySelectorAll(".admin-nav a").forEach(i=>i.classList.remove("active")),e.target.classList.add("active"),this.currentTab=e.target.dataset.tab,this.render()})})}render(){const t=document.getElementById("admin-main");this.currentTab==="settings"?(t.innerHTML=this.renderSettingsTab(),this.setupSettingsListeners(),this.drawPreviewWheel("previewCanvas")):this.currentTab==="appearance"?(t.innerHTML=this.renderAppearanceTab(),this.setupAppearanceListeners(),this.drawPreviewWheel("appearancePreviewCanvas")):this.currentTab==="entries"?(t.innerHTML=this.renderEntriesTab(),this.setupEntriesListeners()):this.currentTab==="integration"&&(t.innerHTML=this.renderIntegrationTab(),this.setupIntegrationListeners())}renderSettingsTab(){return`
    ...
    `,document.getElementById("editModal").classList.add("active"),document.getElementById("seg-prob").addEventListener("input",i=>{document.getElementById("seg-prob-val").textContent=i.target.value}),document.getElementById("seg-type").addEventListener("change",i=>{const n=document.getElementById("seg-val-group"),a=document.getElementById("seg-coupon-group"),o=document.getElementById("seg-ikas-campaign-group"),s=i.target.value==="noLuck",r=i.target.value==="freeShipping";n&&(n.style.display=s||r?"none":"block"),a&&(a.style.display=s?"none":"block"),o&&(o.style.display=s?"none":"block")}),this.populateIkasCampaignSelect(e.ikasCampaignId),document.getElementById("cancelSegBtn").addEventListener("click",()=>document.getElementById("editModal").classList.remove("active")),document.getElementById("saveSegBtn").addEventListener("click",async()=>{var n,a,o;const i={id:this.editingSegmentId||$(),label:document.getElementById("seg-label").value||"Yeni Ödül",icon:document.getElementById("seg-icon").value||"",color:document.getElementById("seg-color").value||"#1E3A8A",textColor:document.getElementById("seg-textcolor").value||"#FFFFFF",discountType:document.getElementById("seg-type").value||"percentage",discountValue:parseInt((n=document.getElementById("seg-value"))==null?void 0:n.value)||0,couponCode:((a=document.getElementById("seg-coupon"))==null?void 0:a.value)||null,ikasCampaignId:((o=document.getElementById("seg-ikas-campaign"))==null?void 0:o.value)||null,probability:parseInt(document.getElementById("seg-prob").value)||10};if(this.editingSegmentId){const s=this.config.segments.findIndex(r=>String(r.id)===String(this.editingSegmentId));s!==-1&&(this.config.segments[s]=i)}else this.config.segments.push(i);document.getElementById("editModal").classList.remove("active"),await this.saveAndRender({segments:this.config.segments})})}async fetchIkasCampaigns(){if(this._ikasCampaigns)return this._ikasCampaigns;const t=g();try{const e=await fetch(`${t}/api/admin/ikas/campaigns`,{headers:{Authorization:`Bearer ${u()}`}});if(e.ok){const i=await e.json();return this._ikasCampaigns=i.campaigns||[],this._ikasCampaigns}}catch{}return[]}async populateIkasCampaignSelect(t,e=!1){const i=document.getElementById("seg-ikas-campaign"),n=document.getElementById("seg-ikas-campaign-hint");if(!i)return;const a=await this.fetchIkasCampaigns(),o=document.getElementById("seg-ikas-campaign");if(o){if(a.length===0){if(!e){n&&(n.textContent="Yükleniyor... (backend uyanıyor olabilir)"),this._ikasCampaigns=null,setTimeout(()=>this.populateIkasCampaignSelect(t,!0),4e3);return}if(n){n.innerHTML=`Kuponu olan bir İkas kampanyası bulunamadı (kuponsuz kampanyalar burada listelenmez). <a href="#" id="retryIkasCampaigns" style="color:var(--cark-primary,#ffd700);text-decoration:underline;">tekrar dene</a>. Yoksa İkas Builder'da kampanyanıza bir kupon kodu ekleyip buradan seçebilir, ya da (önerilen) yukarıya sabit bir kupon kodu girebilirsiniz.`;const s=document.getElementById("retryIkasCampaigns");s&&s.addEventListener("click",r=>{r.preventDefault(),n.textContent="Yükleniyor...",this._ikasCampaigns=null,this.populateIkasCampaignSelect(t,!0)})}return}a.forEach(s=>{const r=document.createElement("option");r.value=s.id,r.textContent=s.title,String(s.id)===String(t)&&(r.selected=!0),o.appendChild(r)})}}drawPreviewWheel(t="previewCanvas",e=null){const i=document.getElementById(t);if(!i)return;const n={...C.theme,...this.config.theme||{},...e||{}},a=i.getContext("2d"),o=i.width/2,s=i.height/2,r=Math.min(o,s)-10;if(a.clearRect(0,0,i.width,i.height),!this.config.segments.length)return;const m=this.config.segments.reduce((c,l)=>c+l.probability,0)||1;let d=-Math.PI/2;const v=document.getElementById("previewStats");v&&(v.innerHTML=`Toplam Ağırlık: <span>${m}</span>`),a.beginPath(),a.arc(o,s,r,0,Math.PI*2),a.fillStyle="#1a1a2e",a.fill(),a.strokeStyle=D(n.primaryColor,.3),a.lineWidth=2,a.stroke();for(const c of this.config.segments){const l=c.probability/m*2*Math.PI,p=d+l;a.beginPath(),a.moveTo(o,s),a.arc(o,s,r-10,d,p),a.closePath(),a.fillStyle=c.color,a.fill(),a.strokeStyle="rgba(255,255,255,0.3)",a.lineWidth=1,a.stroke();const b=d+l/2;a.save(),a.translate(o,s),a.rotate(b),a.textAlign="center",a.textBaseline="middle",a.font="bold 12px sans-serif",a.fillStyle=c.textColor||"#FFF",a.fillText(c.label||"",r*.6,0),a.restore(),d=p}a.beginPath(),a.arc(o,s,r*.2,0,Math.PI*2),a.fillStyle=n.bgDark,a.fill(),a.strokeStyle=n.primaryColor,a.lineWidth=2,a.stroke()}renderEntriesTab(){return`
    ...
    `}setupEntriesListeners(){var t,e;this.loadEntries(),(t=document.getElementById("exportBtn"))==null||t.addEventListener("click",async()=>{const i=g();if(u()){const a=await(await fetch(`${i}/api/admin/entries/export`,{headers:{Authorization:`Bearer ${u()}`}})).blob(),o=URL.createObjectURL(a),s=document.createElement("a");s.href=o,s.download=`cark-katilimcilar-${new Date().toISOString().split("T")[0]}.csv`,document.body.appendChild(s),s.click(),document.body.removeChild(s),URL.revokeObjectURL(o)}else L();this.showToast("CSV dosyası indiriliyor")}),(e=document.getElementById("clearEntriesBtn"))==null||e.addEventListener("click",async()=>{if(!confirm("Tüm katılımcı verileri silinecek. Emin misiniz?"))return;const i=g();u()?await fetch(`${i}/api/admin/entries`,{method:"DELETE",headers:{Authorization:`Bearer ${u()}`}}):A(),this.entriesPage=1,this.loadEntries(),this.showToast("Veriler silindi")})}async loadEntries(){var r,m;const t=document.getElementById("entriesContainer"),e=g(),i=50;this.entriesPage=this.entriesPage||1;let n=[],a={total:0,today:0,mostWon:"-"},o=0;if(u())try{const d=u(),[v,c]=await Promise.all([fetch(`${e}/api/admin/entries?page=${this.entriesPage}&limit=${i}`,{headers:{Authorization:`Bearer ${d}`}}),fetch(`${e}/api/admin/stats`,{headers:{Authorization:`Bearer ${d}`}})]);if(v.ok){const l=await v.json();n=l.entries||[],o=l.total||0}c.ok&&(a=await c.json())}catch{}else{n=P();const d=new Date().toISOString().split("T")[0];a.total=n.length,a.today=n.filter(c=>{var l;return(l=c.timestamp)==null?void 0:l.startsWith(d)}).length;const v=n.map(c=>c.prize).filter(Boolean);if(v.length>0){const c=v.reduce((l,p)=>(l[p]=(l[p]||0)+1,l),{});a.mostWon=Object.keys(c).reduce((l,p)=>c[l]>c[p]?l:p)}}if(document.getElementById("stat-total").textContent=a.total,document.getElementById("stat-today").textContent=a.today,document.getElementById("stat-mostwon").textContent=a.mostWon,document.getElementById("stat-broken").textContent=a.brokenCoupons??"-",u()?o===0:n.length===0){t.innerHTML='<div class="empty-state">Henüz kimse çarkı çevirmedi.</div>';return}t.innerHTML=`
      ...
      `:""}
    `,(r=document.getElementById("entriesPrevBtn"))==null||r.addEventListener("click",()=>{this.entriesPage=Math.max(1,this.entriesPage-1),this.loadEntries()}),(m=document.getElementById("entriesNextBtn"))==null||m.addEventListener("click",()=>{this.entriesPage+=1,this.loadEntries()})}renderIntegrationTab(){var i;const t=M(this.config,g(),(i=this.store)==null?void 0:i.slug),e=F();return`
