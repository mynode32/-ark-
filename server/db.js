...
export async function ensureSchema() {
  ...
  `);
  await query('CREATE INDEX IF NOT EXISTS operational_events_type_created_idx ON operational_events(event_type, created_at DESC)');
  await query(`
  ...
  `);
  await query('CREATE INDEX IF NOT EXISTS job_runs_name_started_idx ON job_runs(job_name, started_at DESC)');
  await query(`
  ...
  `);
  await query('CREATE INDEX IF NOT EXISTS widget_config_snapshots_store_created_idx ON widget_config_snapshots(store_id, created_at DESC)');
